...
using System;
using NGUI.Scripts.Interaction;
...
public class UILabel : UIWidget
{
    ...
    public Color effectColor2
    {
        get
        {
            return mEffectColor2;
        }
        set
        {
            if (mEffectColor2 != value)
            {
                mEffectColor2 = value;
                if (mEffectStyle != Effect.None) shouldBeProcessed = true;
            }
    ...
    public int quadsPerCharacter
    {
        get
        {
            ...
            else if (mEffectStyle == Effect.Outline_Shadow || mEffectStyle == Effect.Outline_Shadow2) return 12;
            return 1;
        }
    }
        ...
        if (effectStyle != Effect.None)
        {
            ...
            if ((effectStyle == Effect.Outline) || (effectStyle == Effect.Outline8))
            {
                ...
                if (effectStyle == Effect.Outline8)
                {
                    ...
                    ApplyShadow(verts, uvs, cols, offset, end, -pos.x, 0, mEffectColor);
                    ...
                    ApplyShadow(verts, uvs, cols, offset, end, 0, pos.y, mEffectColor);
            ...
            else if (effectStyle == Effect.Outline_Shadow)
            {
                offset = end;
                end = verts.Count;
                var outline_pos = this.mEffectDistance2;

                var verts2 = verts;
                var uvs2 = uvs;
                var cols2 = cols;

                ApplyShadow(verts2, uvs2, cols2, offset, end, -outline_pos.x, outline_pos.y, mEffectColor2);
                ...
                if (onPostFill != null)
                {
                    onPostFill(this, offset, verts2, uvs2, cols2);
                }
            }
            else if (effectStyle == Effect.Outline_Shadow2)
            {
                var outline_pos = this.mEffectDistance2;

                offset = end;
                end = verts.Count;
                ApplyShadow(verts, uvs, cols, offset, end, -outline_pos.x, -pos.y, mEffectColor);
                offset = end;
                end = verts.Count;
                ApplyShadow(verts, uvs, cols, offset, end, outline_pos.x, -pos.y, mEffectColor);
                ...
                ApplyShadow(verts2, uvs2, cols2, offset, end, -outline_pos.x, 0, mEffectColor2);
                offset = end;
                end = verts.Count;
                ApplyShadow(verts2, uvs2, cols2, offset, end, outline_pos.x, 0, mEffectColor2);
                offset = end;
                end = verts.Count;
                ApplyShadow(verts2, uvs2, cols2, offset, end, 0, -outline_pos.y, mEffectColor2);
                offset = end;
                end = verts.Count;
                ApplyShadow(verts2, uvs2, cols2, offset, end, 0, -outline_pos.y, mEffectColor2);
    ...
    public void ApplyShadow(List<Vector3> verts, List<Vector2> uvs, List<Color> cols, int start, int end, float x, float y, Color color)
    {
        Color c = color;
        c.a *= finalAlpha;
