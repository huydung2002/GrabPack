...
public class UIWidget : UIRect
{
	...
	protected override void OnAnchor()
	{
			...
			leftAnchor.target == topAnchor.target)
		{
			...
			bool ignoreNotch =  this.CompareTag("IgnoreNotch");
			if (!ignoreNotch && (leftAnchor.target.GetComponent<UIPanel>()))
			{
				//if (this.name == "Background") 
				//Debug.Log(this.name + leftAnchor.relative + " " + rightAnchor.relative + " " + topAnchor.relative + " " + bottomAnchor.relative);
				if (leftAnchor.relative == 0 && rightAnchor.relative == 1 && topAnchor.relative == 1 && bottomAnchor.relative == 0)
				{
					//full screen object (background)
				}
				else
				{

					if (leftAnchor.relative == 0 || rightAnchor.relative == 0)
					{
						offsetX = UIRoot.IPHONE_X_OFFSET_LEFT;
					}
					if (leftAnchor.relative == 1 || rightAnchor.relative == 1)
					{
						offsetX = UIRoot.IPHONE_X_OFFSET_RIGHT;
					}
					if (topAnchor.relative == 1 || bottomAnchor.relative == 1)
					{
						offsetY = UIRoot.IPHONE_X_OFFSET_TOP;
					}
					if (topAnchor.relative == 0 || bottomAnchor.relative == 0)
					{
						offsetY = UIRoot.IPHONE_X_OFFSET_BOTTOM;
					}
				}
			}
			...
			if (sides != null)
			{
				lt = NGUIMath.Lerp(sides[0].x, sides[2].x, leftAnchor.relative) + leftAnchor.absolute + offsetX;
				rt = NGUIMath.Lerp(sides[0].x, sides[2].x, rightAnchor.relative) + rightAnchor.absolute + offsetX;
				bt = NGUIMath.Lerp(sides[3].y, sides[1].y, bottomAnchor.relative) + bottomAnchor.absolute + offsetY;
				tt = NGUIMath.Lerp(sides[3].y, sides[1].y, topAnchor.relative) + topAnchor.absolute + offsetY;
				mIsInFront = true;
			}
			else
			{
				...
				lt = lp.x + leftAnchor.absolute + offsetX;
				bt = lp.y + bottomAnchor.absolute + offsetX;
				rt = lp.x + rightAnchor.absolute + offsetY;
				tt = lp.y + topAnchor.absolute + offsetY;
